...
> **人选题，助手只打当前 `/模块`。不是全自动 agent，不能替你输密码、开监听、点确认。**

本项目面向**授权红队 / OSCP+ / HTB** 等合法场景，提供半自动红队工作流。

做成半自动而不是全自动，是因为真实红队中，合格的渗透人员必须根据现场情况研判并选择下一步，而不是把整条链路交给 AI。AI 可能执行不该执行的动作，而在真实任务中无法承担责任。

仓库集成了实战中常用的技能模块、近年高成功率利用链，以及 2025–2026 较新的技术路径，并会持续更新。
